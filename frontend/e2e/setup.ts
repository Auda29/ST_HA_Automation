...
async function checkHAHealth(): Promise<boolean> {
  try {
    ...
    // 200 OK would mean HA is running with auth disabled
...
async function startHAContainer(): Promise<void> {
  ...
  if (!existsSync(composeFile)) {
    console.warn(
      "⚠️  docker-compose.test.yml not found, assuming HA is already running",
    );
    return;
  }
  ...
  try {
    ...
    execSync('docker ps --filter name=ha-test --format "{{.Names}}"', {
      encoding: "utf-8",
    });
    ...
    console.warn(
      "⚠️  Could not start HA container, assuming it is already running:",
      error,
    );
  }
}
